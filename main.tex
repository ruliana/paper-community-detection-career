...
Sendo $q_{i\curvearrowright}$, a probabilidade de saída da comunidade $i$, expande-se a Equação~\ref{eq:map-equation} em
...
Para isso, duas alterações são feitas na Equação de Mapa: ajuste de probabilidade para considerar nós em múltiplas comunidades, probabilidades diferentes para entrada e saída do \textit{codebook} índice.

Para calculá-las, transforma-se a matriz de adjacência de um grafo ponderado e direcionado $\mathcal{W}$ na matriz estocástica colunar $w$, onde a soma dos pesos de entrada do nó (coluna) é normalizado para que some 1.

Cada elemento $w_{\alpha \beta}$ dessa matriz representa a probabilidade de um \textit{random walker} chegar em $\beta$ a partir do nó $\alpha$ e são definidos como:

\begin{align*}
w_{\alpha \beta} = 
    \begin{cases*}
        0, & \text{se não há conexão entre $\alpha$ e $\beta$,}\\
        \frac{\mathcal{W}_{\alpha \beta}}
               {\sum_\beta \mathcal{W}_{\alpha \beta}},
                & \text{caso contrário.}
    \end{cases*}
\end{align*}

Na equação original, a probabilidade estacionária $p_\alpha$ de um \textit{random walker} estar em um nó $\alpha$ é dada pelo equação PageRank resolvida pelo Método das Potências:

\begin{equation*}
p_\alpha = \sum_{\beta \to \alpha} (1 - \tau)  \frac{p_\beta}{d_\beta} + \frac{1}{n} \tau
\end{equation*}

TODO: passar essa equação para mesma notação que a equação de múltiplas comunidades (usando a matrix estocástica $W$).

Na variação da Equação de Mapa com sobreposição, a probabilidade estacionária $p_{\alpha_i}$ para todo nó $\alpha$ pertencente à comunidade $i$ é resolvida pelo sistema de equações
