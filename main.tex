...
Portanto, a identificação de comunidades do algoritmo Infomap identifica fronteiras de carreiras em uma rede em que as conexões representam o fluxo de profissionais entre ocupações\footnote{Ronie: acho que dá para explicar melhor, mas a linha de raciocínio é essa.}.
...
Para compreender o algoritmo, é preciso entender como um trajeto aleatório em uma rede pode ser representado e como a Descrição de Comprimento Mínimo pode ser usada na detecção de comunidades.
...
Na rede da Figura~\ref{fig:graph01}, se cada nó for representado por uma sequência de bits, podemos medir quanta informação é necessária para representar o caminho feito pelo \textit{random walker}. Usando a codificação de \cite{Huffman1952-ak}, a mesma rede é representada pela Figura~\ref{fig:graph03} e o caminho da Figura~\ref{fig:walk01} usa 2880 bits.

Se a rede for dividida em grupos, como anteriormente, e usarmos uma sequência única de bits para marcar quando o \textit{walker} entra ou sai de um grupo, podemos reaproveitar a nomenclatura entre os nós dos grupos, compactando a informação necessária para representar o caminho. A Figura~\ref{fig:graph04} mostra os códigos para a entrada de cada grupo (\enquote{0}, \enquote{10} e \enquote{11}) e os códigos de saída de cada grupo (\enquote{111}), bem com os códigos reaproveitados em cada um deles. Com essa codificação, o caminho da Figura~\ref{fig:walk01} usa 1776 bits.
