...
% Ronie: é... depois de um início meio diferente, a carreira dele realmente é linear. Vou procura um outro exemplo melhor e conhecido.
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.9\linewidth]{pareto.png}
  \caption{Gráfico de Pareto com a distribuição de grau. Os nós estão ordenados pelo grau de maneira decrescente. O eixo esquerdo, em escala logarítmica, possui o grau do nó, enquanto o eixo direito possui a porcentagem acumulada. As marcações separam o quartil dos nós de maior grau (1º quartil), a mediana e o quartil de menor grau (3º quartil).}
  \label{fig:pareto}
\end{figure}

A rede de ocupações possui a distribuição de grau exibida no gráfico de Pareto da Figura~\ref{fig:pareto}. Ela possui uma grande concentração de conexões em poucos nós, formando \textit{hubs}. Os 25\% nós de maior grau são responsáveis por 89\% das conexões da rede. O nó de maior grau possui conexão com 6308 dos 7450 nós da rede, ou seja, ele conecta cerca de 85\% das ocupações.

Observando a Figura~\ref{fig:pareto} é possível notar que quase $1/4$ das ocupações possui apenas uma conexão (1689 delas, $\approx$ 23\%). Independente do algoritmo de detecção de comunidades, esses nós serão levados a participar da mesma comunidade que seu parceiro. Essa configuração força o tamanho das comunidades para cima se elas contiverem um \textit{hub}. Por exemplo, mesmo que o nó de maior grau tenha preferência de conexão por outros nós de alto grau, ainda assim ele estaria conectado a pelo menos 8\% dos nós de grau 1. Ou seja, a menor comunidade possível que inclua o nó de maior grau possuirá $\approx$ 600 nós.

Essa topologia, por si só, fornece indícios que a \enquote{Carreira sem Fronteiras} de fato acontece, mas que ela não significa um movimento livre entre ocupações. A transição mais comum acontece primeiramente para uma ocupação \enquote{coringa} (\textit{hub}) antes de uma segunda transição mais específica.

A topologia de \textit{hubs} faz esperar que os grupos formados possuam um nó central altamente conectado, por onde passa o maior fluxo de profissionais.
...
\begin{table}
  ...
  \begin{tabular}{@{} l r r @{}}
    \toprule
    Método           & Compressão & Partições  \\
    \midrule
    Simples          & 14\%       & 123     \\
    Hierárquico      & 14\%       & 123     \\
    Com sobreposição & 44\%       & 930     \\
    \bottomrule
  \end{tabular}
...
\begin{table}
  ...
  \begin{tabular}{@{} l r r r r @{}}
    ...
    \cmidrule(r){4-5}
    Rede                                   & Nós  & Conexões & Simples & $\Delta$ Sobreposição \\
    \midrule
    \rowcolor{yellow!25} Mapa de Carreiras & 7450 & 90800    & 14\%    & 30\%                  \\
    Estradas Europeias                     & 1018 & 1274     & 46\%    & 10\%                  \\
    Grade de Energia Leste dos EUA         & 4941 & 6994     & 53\%    & 8\%                   \\
    Doenças Humanas                        & 516  & 1188     & 46\%    & 3\%                   \\
    Coautoria de Artigos                   & 552  & 1317     & 49\%    & 2\%                   \\
    Rotas Aéreas                           & 3618 & 14142    & 31\%    & 1\%                   \\
    Blogs Políticos (EUA)                  & 1222 & 16714    & 4\%     & < 1\%                 \\
    Blogs Políticos (Suécia)               & 855  & 10315    & < 1\%   & < 1\%                 \\
    Rede Neural \textit{C. elegans}        & 297  & 2345     & 1\%     & < 1\%                 \\
    \bottomrule
  \end{tabular}
  \caption{Comparativo da estrutura do Mapa de Carreiras (em destaque) com outras redes~\cite{Viamontes_Esquivel2011-it}. \textit{Compressão Simples} se refere à compressão que o Infomap obteve no melhor particionamento simples da rede (sem sobreposição ou hierarquia de partições). \textit{Compressão $\Delta$ Sobreposição} se refere ao ganho na compressão obtido pela sobreposição de partições em relação à compressão simples.}
...
% Na verdade, são todas as ocupações extraídas nessa versão. Mudei o texto, acha que ficou mais claro?
...
3 - Existem indícios de que as divisões fazem sentido: \\
 - A carreira de \enquote{mídia} foi separada pelo algoritmo, a direção predominante dos fluxos trás exatamente a sequência esperada de \enquote{evolução profissional}.
...
% Justifiquei no final do capítulo anterior, veja se faz sentido.
